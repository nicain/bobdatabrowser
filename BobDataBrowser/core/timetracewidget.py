...
class TimeTraceWidget(object):
    ...
    def initialize(self):
        ...
        def start_change(attr, old, new):
            self.app.time_range_manager.set_time_range_start(new)
        self.figure.x_range.on_change('start', start_change)

        def end_change(attr, old, new):
            self.app.time_range_manager.set_time_range_end(new)
    ...
    def set_active_cell(self, active_cell_manager):
        ...
        time_range = self.app.time_range_manager.time_range
        ...
        self.app.time_range_manager.set_time_range(time_range)
    ...
    def set_time_range(self, time_range_manager):
        ...
        if self.scrubber_bar.location > self.figure.x_range.end:
            self.app.active_time_index_manager.set_active_time_index(int(self.figure.x_range.end - .01 * (self.figure.x_range.end - self.figure.x_range.start)))
